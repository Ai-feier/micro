...
func TestRedisFixWindowLimiter_BuildServerInterceptor(t *testing.T) {
	...
	interceptor := NewRedisFixWindowLimiter(rdb, "user-service", 3*time.Second, 1).BuildServerInterceptor()
	cnt:=0
...
func TestFixWindowLimiter_LimitUnary(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			defer tc.after(t)
			limiter := NewRedisFixWindowLimiter(rdb, tc.key, tc.interval, tc.rate)
			limit, err := limiter.limit(context.Background())
